...
class Paddle:
	...
	def draw(self):
		...
		pygame.draw.rect(screen, RED if not self.winner else BLUE, [2, self.y+2, 20-4, 100-4])  # Inner rectangle
...
def displayNetwork(layer_structure, coefs=testCoefs, command=0):
    # Stores the largest coefficient, so we can scale the thicknesses accordingly.
    ...
    # Label inputs (adjusted for bottom left positioning)
    for i in range(layer_structure[0]):
        font = pygame.font.SysFont('Calibri', 30, False, False)
        text = font.render(inputs[i], True, TEXT)
        screen.blit(text, [size[0]*0.5-10, size[1] - (i+1) * int(height / (layer_structure[0]+2)+5)])

    # Label outputs (adjusted for bottom left positioning)
    for i in range(layer_structure[-1]):
        font = pygame.font.SysFont('Calibri', 30, False, False)
        text = font.render(str(outputs[i]), True, TEXT)
        screen.blit(text, [size[0] - 80, size[1] - (i+1) * int(height / (layer_structure[-1]+2)+5)])
    ...
    for i in range(layerCount):
        ...
        for j in range(layer_structure[i]):
            layer_circle_positions.append((size[0]*0.5 + (i+1) * xspacing, size[1] - (j+1) * yspacing))
    ...
    for layer in circle_positions:
        for circle_pos in layer:
            ...
            pygame.draw.circle(screen, WHITE, circle_pos, 16, 0)
...
for i in range(PLAYERS):
	...
	#Why do we need to create a ball for each paddle?
	balls.append(Ball())
	if i%2 == 0:
		paddles[i].playerLeft = False
...
while not done:
	...
	for i, paddle in enumerate(paddles):
		...
		input = np.array([[paddle.y, balls[i].x, balls[i].y, balls[i].xspeed]])
